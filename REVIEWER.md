...
- Spec documents carry the human's words: spec content with no recorded approval behind it, or that reads as the agent's analysis rather than the human's statement, is a finding; the agent's spec work belongs in the tasking file (`design/AGENT_SPECS.md` § Tasking file).
